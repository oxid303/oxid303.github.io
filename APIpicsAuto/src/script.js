$(() => {
  ...
  function viewMobile() {
    if (width > height) {
      ...
      $('.autoplay-container').css('display', 'none');
      $('.big-image').css({
      ...
      $('.autoplay-container').css('display', 'block');
      $('.big-image').css({
        'max-height': height - 215 + 'px',
        position: 'absolute',
        top: '105px',
        left: '50%',
  ...
  function runIfAutoplay() {
    if (!autoplay) runSetInterval()
    else runClearInterval();
    $('.autoplay').text(autoplay ? 'PLAY' : 'STOP');
    autoplay = !autoplay;
  }

  $('.autoplay').click(() => runIfAutoplay());
  if (mobile) $('.big-image-container').on('click', 'img', () => runIfAutoplay());
  ...
  function timeSet(timeWay) {
    switch (timeWay) {
      case 'up':
        ...
        break;
      case 'down':
        if (seconds > 3 && seconds <= 10) seconds -= 1;
        if (seconds > 10) seconds -= 5;
        break;
      default:
        break;
    }
    $('.autoplay').html(`${seconds} seconds`);
    viewSeconds();
    if (autoplay) runSetInterval();
  }
  ...
  function viewSeconds() {
    clearTimeout(runViewSeconds);
    runViewSeconds = setTimeout(() => {
